...
As such, you can send a line to the terminal without explicitly selecting it
(this also works with copy-paste/cut-paste within the editor!).
...
Potential problems will show up as colored squigly lines below the words or lines
in question.
...
``bash
fastqc --threads 8 --outdir="$output_dir" "$input_file"
```
...
```bash
# This script will create a genome "index" for STAR, that will be used in the next step
...
# This script will align a FASTQ file to the genome index created in the previous step
...
and would fail because of the missing output from the first step.
...
- A "runner" script that orchestrates the batch job submission of these individual steps.
  This script should be run at once in its entirety.
...
At the end of the Slurm log file, our script listed _FastQC_'s output files
...
To view FastQC's HTML output files, we unfortunately need to _download_ them
with the older version of VS Code that's installed at OSC.
