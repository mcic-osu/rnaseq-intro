...
The collection of bioinformatics programs at OSC is unfortunately not comprehensive,
...
ourselves.
Two common methods are using the _Conda_ software management and containers &mdash;
we will talk about loading MCIC's _Conda_ enviroments,
while there is at-home reading about installing your own software with _Conda_,
...
below, we'll search for "miniconda" with each of these commands:
...
use `module list` &mdash;
the output will also include _automatically_ loaded modules:
...
The `miniconda3` module will allow us to use Conda software environments,
...
The at-home reading includes installing software yourself with Conda,
and using containers.
...
its name is conveniently displayed in front of our prompt, as depicted above.
:::

After we have activated the `multiqc` environment, 
we should be able to actually use the program.
To test this, we'll simply run it with the `--help` option like we did with
_FastQC_:
...
_Conda_ will by default only keep **a single environment activated**.
...
Note that it's `conda activate --stack` and not `source activate --stack`!
...
- In the example above,
  the `set -euo pipefail` line was moved to _below_ the `source activate` command,
...
[^4]: Unless you first deactivate any active environments in your script,
      which the "MCIC scripts" that we'll talk about later do.
...
We first have to do some _one-time_ configuration[^5],
...
[^5]: That is, these settings will be saved somewhere in your OSC home directory,
      and you never have to set them again unless you need to make changes.

We can do this with the `config` subcommand &mdash;
...
<p align="center"><img src=img/conda_website_version.png width="70%"></p>
...
- Export a plain-text "YAML" file that contains the instructions to recreate
  your environment (useful for reproducibility!)  
...
Luckily, _Apptainer_ can also work with _Docker_ containers. 
...
<img style="border:1px solid #4e524f;" src=img/biocontainers_search.png width="70%">
...
<figure><p align="center">
<img style="border:1px solid #4e524f;" src=img/biocontainers_match.png width="80%">
</p></figure>
...
<figure><p align="center">
<img style="border:1px solid #4e524f;" src=img/biocontainers_download.png width="80%">
...
pertains to the version of TrimGalore,
...
<figure><p align="center">
<img style="border:1px solid #4e524f;" src=img/biocontainers_versions.png width="80%">
...
When you've found a container image that you want to use, copy its URL.
...
- We will use the more up-to-date `apptainer` command[^6]
...
[^6]: Though note that as of September 2023,
...
And the `--help` option above can also simply be replaced by a host of other 
_TrimGalore_ options and arguments
so as to actually trim a pair of FASTQ files, i.e. with input and output files.
...
- Try a container when installing a program through Conda fails,
  or your environment misbehaves (e.g., memory errors with dumped cores).
