...
infected with malaria-causing *Plasmodium* protozoans --- specifically,
it compares mosquitoes according to:
...
I have **subset** the data to omit the 21-day samples.
I have also already ran the [nf-core rnaseq pipeline](https://nf-co.re/rnaseq),
so for these sessions, we have the following input files: 

- A gene count table produced by the nf-core rnaseq pipeline.
...
count_table_file <- "/fs/ess/PAS0471/demo/202307_rnaseq/garrigos/salmon.merged.gene_counts_length_scaled.tsv"
metadata_file <- "/fs/ess/PAS0471/demo/202307_rnaseq/garrigos/metadata.tsv"
```
