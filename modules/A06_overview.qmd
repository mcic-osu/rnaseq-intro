...
That is, they have a "command-line interface" (*CLI*) rather than a
...
However, recall that our full dataset also has _much larger_ files.
A _FastQC_ run on one of these will take several minutes,
so our loop might take a while to finish
(and it gets worse,
as some of the other programs we'll run later take much longer than this).
...
[session on shell scripts](./A07_scripts.qmd) &mdash;
...
we can modify our loop as follows to run _FastQC_ through a script,
...
With the script, we've set ourselves up to be able to submit batch jobs,
but running the script in itself does not change much yet.
...
in the [session on Slurm compute jobs](./A09_slurm.qmd).
...
- **Making software available**  
  ...
  [session Software at OSC](./A08_software.qmd).
...
- **Writing shell scripts**  
  ...
  We'll talk about [shell scripts in the next session](./A07_scripts.qmd).
...
- **Submitting scripts to the Slurm job scheduler**  
  ...
  We'll see this in more detail in the [session on Slurm jobs](./A09_slurm.qmd). 
...
When you run it, you should notice a slight pause after the output of each
iterations, and you can see that the output of `date` differs by one second
each time:
