---
title: "Overview of next steps / shell variables & `for` loops"
...
where we start with reads in FASTQ files to produce a table with gene counts.
This will reveal **which fundamentals we still need to cover**
**before we can start running these analyses.**

Then, we'll cover the first of the remaining fundamental topics:
**shell variables** and **`for` loops**.
The next few sessions will cover the remaining ones:

- [Writing shell scripts](./A07_scripts.qmd)
- [Using software at OSC](./A08_software.qmd)
- [Submitting scripts to the OSC job queue](./A09_slurm.qmd).
...
We'll use the program _FastQC_ as an example,
...
We will cover that in more in detail in the session [Software at OSC](A08_software.qmd)
...
```bash
mkdir -p results/fastqc # First create the output dir
...
including an HTML file that contains figures with all the results.

So should we simply repeat this for all our FASTQ files?
I.e., we could run the `fastqc` command 8 times,
...
to pass multiple files to _FastQC_.
...
and can be used with the many programs that can only run one FASTQ file or one
sample at a time.
When using a supercomputer like OSC,
this is also by far the most time-efficient approach,
...
We can avoid having to type a very similar command a whole bunch of times
by using a **loop**.
...
and the loop would be exactly the same regardless of whether we had
8 or 200 FASTQ files!

However, recall that our full dataset also _much larger_ files.
A _FastQC_ run on one of these will take several minutes,
so our loop might take a while to finish
(and some of the other programs we'll run later take much longer than this).
Most of all, when we run the program interactively and consecutively like this,
we would not make use of the supercomputer's strengths at all:
...
What we should do instead is to submit a separate compute job for each _FastQC_ run,
and these can then all run in parallel (i.e., at the same time) on different compute nodes.
To make the switch from interactively running _FastQC_,
...
We will cover the basics of shell scripts in [A07 - Shell scripts](link) &mdash;
for now, just realize that these scripts are small text files with the extension
`.sh` that basically contain the same kind of Unix shell code we've been running
interactively, with some added bells and whistles.

If for now we just assume we had created such a script,
we can modify our loop as follows to run _FastQC_ through a script instead of
including the command to run the program directly in the loop:
...
# [...]
Started analysis of ASPC1_G31V_R2.fastq.gz
Approx 5% complete for ASPC1_G31V_R2.fastq.gz
# [...]
Started analysis of Miapaca2_A178V_R1.fastq.gz
Approx 5% complete for Miapaca2_A178V_R1.fastq.gz
# [...]
```

However,
running that loop would still make _FastQC_ run _sequentially_ on the node that
we are at,
...
- **Making software available**  
  ...
  [Software at OSC session](./A08_software.qmd).
...
- **Writing shell scripts**  
  ...
  (It's also a good way to keep your code organized and clear.)

- **Submitting scripts to the Slurm job scheduler**  
  Submitting scripts as Slurm jobs allows us, for example,
  to run the same analysis for different samples simultaneously, in parallel.
...
put another way, it takes aways the special meaning of the spaces (as _separators_)
...
as many times as there are items in the collection:
...
| Keyword | Purpose	                            |
|---------|-------------------------------------|
| `for`	  | After `for`, we set the **variable name** (an arbitrary name, whatever we choose)
...
having the last value that was assigned to it in the loop:
...
demonstrates that the loop is being executed sequentially: xxx
...
- Or FASTQ files are in sample-specific folders:
...
One practical example of this is that you might want to automatically 
include the current date in a file name.
...
```bash
nlines=$(zcat data/fastq/ASPC1_A178V_R1.fastq.gz | wc -l)
