...
While not always strictly necessary, adding a _shebang_ line is good practice,
...
(but e.g. misspelled its name, or forgot to assign it altogether).
...
In the next section,
we'll see what actually happens when we pass arguments to a script on the command line.
...
Unless we explicitly include code in the script to _do_ something with
these variables, nothing else happens. 
