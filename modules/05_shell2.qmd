...
subtitle: "Working with dirs and files"
...
The `wc` (short for `w`ord`c`ount) command can count (e.g.) the number of
...
For more, see the [reference page](../info/shell_commands.qmd).

#### Command history and moving across the line faster

It's common to want to repeat commands and even more common to make slight
changes to a previously issued command.
Besides <kbd>Tab</kbd> for tab completion,
another ubiquitous pair of shell keyboard shortcuts are
<kbd>&#8679;</kbd> / <kbd>&#8681;</kbd>
to cycle through your **command history**.

If you hit the <kbd>&#8679;</kbd> (up arrow) once,you'll get your most recent command,
and if you keep hitting it, you'll go further back;
the<kbd>&#8681;</kbd> (down arrow) will go the other way.
...
that _requires_ an argument[^1], something odd happens &mdash;
you move to a new line in the terminal but don't get your regular prompt
back; it appears to "hang". Try it:

[^1]: Though we will see later that you can also "pipe in" input to `wc` and
...
Another type of situation where <kbd>Ctrl</kbd>+<kbd>C</kbd> is very handy is
...
for instance, we reference a variable with the name `x` as `$x`[^1].

[^1]: The advantage of this is that it makes it very easy to recognize variables
...
when we have our shell scripts report progress and the values of variables,
and so on.

If we use echo with a(n environment) variable like `$USER`,
it will print its value:
...
Why do we need to learn this when VS Code and the OnDemand
...
this is how I usually structure my research project dirs:
...
This command isn't as commonly used because we usually create new files on
the fly with command output redirection, which we'll learn about below.
:::
...
we need to use its `-r` option[^2].
We'll use that option to copy the dir with FASTQ files:

[^2]: `-r` is a common option name for recursive behaviour among Unix commands
...
that are used to separate commands, options, and arguments,
with the latter often being one or more file names.

For example, one of our earlier `mkdir` commands,
`mkdir data metadata results scripts sandbox`,
created five separate dirs and not one dir with four spaces in it.
...
In any context that we want to refer to file or dir names with spaces in them,
we'll need quotes or escaping:
...
**avoid spaces in file names** when working at OSC but more generally
in a project that uses the Unix shell or in fact that uses any coding.
If you need to separate words, then use underscores (`_`), dashes (`-`),
and/or capitalization &mdash; for example:
...
- Finally, it may be assuring to learn that
  **you will not be able to remove other people's files at OSC**
  (since you do not have "write permission" for them)[^3].
  For example, the file below was created by me and you won't be able to remove
  (or edit) it:

[^3]: At least not by default, the person who created the file can change this.
...
As such, we can use shell wildcards with any command that takes (one or) multiple
files as arguments, for example:
...
`*` matches all files by definition,
and can for example be useful if you want to move the _contents_ of a directory
...
Additionally, when you list files in the shell in VS Code,
