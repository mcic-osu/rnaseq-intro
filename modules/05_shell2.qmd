...
Try to think about what that means for a second.
As a consequence,
...
and can for example be useful if you want to copy the entire _contents_ of a
...
For those of you who know some regular expressions from coding in e.g. R,
Python, or advanced usage of GUI-based text editors like jEdit:
wildcards are _conceptually_ similar,
...
We _can_ also use regular expressions in the Shell but this is functionality
that is built in to several specific commands, like `grep` and `sed`,
...
What can we use this functionality for, other than listing matching files?

We already saw one `cp` example above, and indeed,
...
For example, let's take a look at the `README.md` file[^7] in your working dir:
...
Unix commands (and many bioinformatics command-line tools alike) actually separate
"regular" output (called "**standard out**") from error messages 
("**standard error**").

Without redirection, both are printed to screen, so we might never notice.
...
**The error message was not redirected to the output file at all,**
**but only to the screen!**
...
we had both standard out (the file listing of `README.md`)
and standard error (the error message for `non_existent_file`), and:

- Standard out was redirected to the file
- Standard error was still printed to screen.
